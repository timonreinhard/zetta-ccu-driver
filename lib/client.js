...
CcuClient.discover = function (cb) {
  var ssdpClient = new SSDPClient()
  ssdpClient.on('response', function (headers, statusCode, rinfo) {
    if (headers.SERVER === 'HomeMatic') {
      var instance = new CcuClient({
        ccuHostname: rinfo.address
      })
...
CcuClient.prototype.subscribe = function (cb) {
...
CcuClient.prototype.unsubcribe = function (cb) {
